...
如需在落地页显示简短更新日志，在打包前新建 `release-notes/v<version>.json`，提供最多 3 条单语言 `highlights`。打包脚本会自动写入 `docs/downloads.json`，页面同时使用 `publishedAt` 显示发布日期；发布脚本会将同一份摘要写入 GitHub Release 正文。
